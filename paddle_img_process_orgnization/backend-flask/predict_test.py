...
# 创建 config
config = paddle_infer.Config('static\check_object\model.pdmodel',
                             'static\check_object\model.pdiparams')
...
def draw_box(img, results):
    ...
    for i in range(len(results)):
        ...
        # 绘制文本
        draw.text((xmin, ymin), '%s, %0.2f' % ('playground', score), (0, 255, 0), font=font_style)
    cv2.imshow('result', cv2.cvtColor(np.asarray(img), cv2.COLOR_RGB2BGR))


if __name__ == "__main__":
    path = 'ChangeDetection2.png'
